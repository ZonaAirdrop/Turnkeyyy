...
class Turnkey:
    ...
    def save_totals(self):
        data = {
            "total_eth_sent": self.total_eth_sent,
    ...
    def load_totals(self):
        if os.path.exists("totals.json"):
            with open("totals.json", "r") as f:
                data = json.load(f)
                self.total_eth_sent = data.get("total_eth_sent", 0.0)
                self.total_transactions_sent = data.get("total_transactions_sent", 0)
