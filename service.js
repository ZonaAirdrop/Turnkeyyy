...
// Fungsi delay dengan promise (untuk timer)
...
class Turnkey {
  ...
  // Log dengan timestamp sesuai zona waktu Asia/Jakarta
  log(message) {
  ...
  async loadProxies(useProxyChoice) {
    ...
    try {
      if (useProxyChoice === 1) {
        // Ambil proxy secara online menggunakan axios
  ...
  // Menghasilkan address dari private key menggunakan Web3
  ...
  // Buat receiver random (menggunakan fungsi create() milik Web3)
  ...
  // Buat instance Web3 dan cek koneksi (mengambil blockNumber sebagai pengecekan)
  async getWeb3WithCheck(address, useProxy, retries = 3, timeout = 60000) {
    let providerUrl = this.RPC_URL;
    // Jika menggunakan proxy, kamu bisa menambahkan log atau konfigurasi tambahan.
    if (useProxy) {
      const proxy = this.getNextProxyForAccount(address);
      if (proxy) {
        this.log(`Using Proxy: ${proxy} for address ${address}`);
        // NOTE: Penerapan proxy untuk Web3 memerlukan konfigurasi HTTP Agent.
      }
    }
  ...
  // Mendapatkan saldo ETH (dalam ETH)
  ...
  // Melakukan transfer ETH dari akun dengan private key
  ...
  // Menampilkan timer mundur secara asinkron
  async printTimer() {
    ...
    console.log(); // Baris baru setelah timer
  }

  // Menggunakan readline untuk interaksi input user
  async printQuestion() {
    ...
    // Minta input jumlah transaksi per akun
    while (true) {
    ...
    // Minimal jumlah transfer (dalam ETH Sepolia)
    let minAmount;
    while (true) {
    ...
    // Maksimal delay antar transaksi
    while (true) {
  ...
  async main() {
    try {
      // Baca akun dari file accounts.txt (satu private key per baris)
